...
static int cass_best_cpu(struct task_struct *p, int prev_cpu, bool sync)
{
	...
	struct cass_cpu_cand cands[2] = {0}, *best = cands;
	...
	for_each_cpu_and(cpu, &p->cpus_allowed, cpu_active_mask) {
		...
		/*
		 * Check if this CPU is idle. For sync wakes, treat the current
		 * CPU as idle if there is only one task running on it.
		 */
		 ...
		 */
		curr->cap = max(capacity_of(cpu), 1UL);
